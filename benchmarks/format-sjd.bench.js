...
const run = bench(
  [
    ...
    function benchBunyan(done) {
      bunyan.debug('hello %s %j %d', 'world', { obj: true }, 4, {
        another: 'obj'
      })
      process.nextTick(done)
    },
    function benchPino(done) {
      pino.debug('hello %s %j %d', 'world', { obj: true }, 4, {
        another: 'obj'
      })
      process.nextTick(done)
    },
    function benchDebugLevel(done) {
      debugLevel.debug('hello %s %j %d', 'world', { obj: true }, 4, {
        another: 'obj'
      })
      process.nextTick(done)
    },
    function benchPinoAsync(done) {
      pinoAsync.debug('hello %s %j %d', 'world', { obj: true }, 4, {
        another: 'obj'
      })
      process.nextTick(done)
    },
    function benchDebugLevelAsync(done) {
      debugLevelAsync.debug('hello %s %j %d', 'world', { obj: true }, 4, {
        another: 'obj'
      })
      process.nextTick(done)
    }
