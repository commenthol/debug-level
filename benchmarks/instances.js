...
function setup(stream) {
  ...
  const pinoAsync = pinoF(
    { name, level },
    pinoF.destination({ dest: path, fd, sync: true, minLength: 4096 })
  )

  DebugLevel.options({ json: true, colors: false, namespaces: '*' })
  const debugLevel = new DebugLevel(name, {
    level,
    stream,
    sonic: false,
    serverinfo: true,
  ...
  const debugLevelAsync = new DebugLevel(name, {
    level,
    stream,
    sonic: true,
    serverinfo: true,
